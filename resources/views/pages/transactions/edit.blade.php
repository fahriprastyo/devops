...
@section('title')
  Thriftophia | Dashboard Edit Transaksi
@endsection

@section('view_transaction')
  active
@endsection
    ...
    <div class="card">
      ...
      <div class="card-body card-block">
        <form action="{{ route('transactions.update', $item->id) }}" method="POST">
          ...
          <div class="form-group">
            <label for="zip_code" class="form-control-label">Nomor POS</label>
            <input  type="text"
                    name="zip_code"
                    value="{{ old('zip_code') ? old('zip_code') : $item->zip_code }}"
                    class="form-control @error('zip_code') is-invalid @enderror"/>
            @error('zip_code') <div class="text-muted">{{ $message }}</div> @enderror
          </div>
          <div class="form-group">
